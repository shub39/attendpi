...
impl FingerprintSensor {
    ...
    pub fn new(baud_rate: u32, address: u32, password: u32, ) -> Result<Self, serialport::Error> {
        ...
        match sensor.verify_password() {
            Ok(_) => Ok(sensor),
            Err(_) => Err(serialport::Error::new(ErrorKind::InvalidInput, "Invalid password"))
        }
    }

    pub fn get_system_parameters(&mut self) -> Result<SystemParameters, Box<dyn Error>> {
        ...
        if received_payload.is_empty() || received_payload[0] != FINGERPRINT_OK {
            return Err(format!("Get System Parameters command failed with code: {:02X?}", received_payload.get(0).unwrap_or(&0xFF)).into());
        }

        if received_payload.len() < 17 {
            return Err(format!("System parameters payload too short. Expected at least 17 bytes, got {}.", received_payload.len()).into());
        }
    ...
    //verifies password. TODO: Allow users to change password and address
    fn verify_password(&mut self) -> Result<bool, Box<dyn Error>> {
        ...
        match received_payload[0] {
            ...
            other => Err(format!("Received unexpected ACK packet type: {}", other).into())
        }
    }
    ...
    fn receive_exact(&mut self) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    fn parse_response_packet(&self, raw_bytes: &[u8]) -> Result<(u8, Vec<u8>), Box<dyn Error>> {
        if raw_bytes.len() < 12 {
        ...
        if raw_bytes.len() < 9 + expected_payload_len + 2 {
            println!("Received packet shorter than expected payload length. Expected at least {} bytes, got {}.", 9 + expected_payload_len + 2, raw_bytes.len());
